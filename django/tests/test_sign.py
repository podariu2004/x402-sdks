...
from x402_django.sign import SCHEME, canonical, sign, signed_headers

# tests/ -> sdks/django -> sdks -> repo root (same approach as
# sdks/fastapi/tests/test_sign.py).
VECTORS = json.loads(
...
def test_kat_signature_matches_frozen_vector():
    ...
    assert got == "c325bfaf7e66735f1e6a977b4b3b3fa6c9ae98d010123b1f724bed5ce5959ab5"
...
def test_shared_vectors_full_conformance():
    """Cross-SDK drift gate: every body_hash + the KAT signature in the
    shared vector file MUST reproduce exactly (identical assertion shape to
    the FastAPI / Go / Rails / Spring / .NET oracles)."""
    for v in VECTORS["body_hashes"]:
        assert hashlib.sha256(v["body"].encode("utf-8")).hexdigest() == v["sha256"]
